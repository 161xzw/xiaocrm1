...
@RestController
@CrossOrigin
@RequestMapping("crm/Vipcustomer")
public class VipCustomerController {
    ...
    Map<String,Object> findAll(Pages pages){
        ...
        if(key != null){
            if(!key.equals("")){
                //模糊查询
                customers = customerService.findOneVip("%" + key + "%");
                //获取指定客户总数量
                count = customers.size();
            }
        } else {
            customerService.updateVip();
            customers = customerService.findAllVIP((page - 1) * limit,limit);
            //获取客户总数量
            count = customerService.findTotalVip();
        }
        //放入map集合
    ...
    Map<String,Object> findAllqiye(Pages pages){
        ...
        if(key != null){
            if(!key.equals("")){
                //模糊查询
                customers = customerService.findOneVipqiye("%" + key + "%");
                //获取指定客户总数量
                count = customers.size();
            }
        } else {
            customers = customerService.findAllVIPqiye((page - 1) * limit,limit);
            //获取客户总数量
            count = customerService.findTotalVipqiye();
        }
        //放入map集合
        Map<String,Object> map = new HashMap<>();
        map.put("msg",0);
    ...
    Map<String,Object> findAllgeren(Pages pages){
        ...
        if(key != null){
            if(!key.equals("")){
                //模糊查询
                customers = customerService.findOneVipgeren("%" + key + "%");
                //获取指定客户总数量
                count = customers.size();
            }
        } else {
            customers = customerService.findAllVIPgeren((page - 1) * limit,limit);
            //获取客户总数量
            count = customerService.findTotalVipgeren();
        }
        //放入map集合
        Map<String,Object> map = new HashMap<>();
        map.put("msg",0);
